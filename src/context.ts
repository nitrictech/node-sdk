...
export class NitricContext {
  ...
  constructor(requestId: string, source: string, sourceType: NitricSourceType, payloadType: string) {
  ...
  static fromHeaders(headers: Record<string, string>): NitricContext {
    ...
    const sourceType = (headers["x-nitric-source-type"] as NitricSourceType) || "UNKNOWN";
    const source = headers["x-nitric-source"];
    const payloadType = headers["x-nitric-payload-type"];

    return new NitricContext(
      requestId,
      source,
      sourceType,
      payloadType
